...
from src.utils import add_gaussian_noise, apply_gaussian_blur, apply_sharpening, white_out_block
...
class ImagePerturbEnv(gym.Env):
    ...
    def step(self, action: int) -> Tuple[torch.Tensor, float, bool, bool, Dict[str, Any]]:
        if action == 0:
            perturbed_image = add_gaussian_noise(self.image.clone().to(DEVICE))
        elif action == 1:
            perturbed_image = apply_gaussian_blur(self.image.clone().to(DEVICE))
        elif action == 2:
            perturbed_image = apply_sharpening(self.image.clone().to(DEVICE))
        elif action == 3:
            perturbed_image = white_out_block(self.image.clone().to(DEVICE))
