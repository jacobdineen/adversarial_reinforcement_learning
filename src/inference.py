...
test_env = BlockBasedPerturbEnv(
    dataloader=EndlessDataLoader(test_loader), model=model, reward_func=reward_func, steps_per_episode=len(test_loader)
)
...
for idx, (reward, index) in enumerate(zip(top_k_reward, top_k_indices)):
    ...
    raw_output_perturbed_image = model(
        torch.from_numpy(perturbed_image_with_highest_reward).unsqueeze(0).unsqueeze(0).float()
    )
