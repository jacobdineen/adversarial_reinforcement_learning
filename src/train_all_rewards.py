...
if __name__ == "__main__":
    ...
    for reward_func_name, reward_func in reward_functions.items():
        # pylint: disable=R1733
        selected_reward_func = reward_functions[reward_func_name]
        for run in range(k):
            ...
            new_logger = configure(
                LOGGING_OUTPUT_PATH + f"dataset_name={dataset_name}_reward_func={reward_func_name}", ["stdout", "csv"]
            )
            # Training here
    ...
    # plt_helper(f"{LOGGING_OUTPUT_PATH}/progress_{dataset_name}_{reward_func_name}.csv", episodes)
    # plot_rewards_and_cumulative(df["rewards"])
    logging.info(f"Completed {k} training runs")
    plt_helper("src/logs/sb3_log", episodes)
